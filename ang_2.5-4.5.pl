...
where
	s.boundary_id= b.id
        ...
        and stusg.active=2
        and s.active=2
        and (extract(YEAR FROM age(c.dob))*12+extract(month from age(c.dob)))>=30
        and (extract(YEAR FROM age(c.dob))*12+extract(month from age(c.dob)))<=54
        and s.id=?
});
    $children->execute($opts{i});
...
sub format_children_pdf {
    ...
    my $tt = Template->new({
        # DEBUG => 'all',
        INCLUDE_PATH => 'templates/2013/anganwadi/prefilled/',
