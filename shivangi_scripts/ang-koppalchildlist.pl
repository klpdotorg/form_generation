...
from     schools_institution s left outer join schools_staff t on (s.id=t.institution_id and t.staff_type_id=3),
 schools_studentgroup sg, schools_student_studentgrouprelation stusg, schools_student stu, schools_child c left outer join schools_moi_type l on (c.mt_id=l.id) left outer join schools_relations r on (c.id=r.child_id),schools_boundary b, schools_boundary b1, schools_boundary b2, schools_boundary_type bt  where s.boundary_id = b.id and b.parent_id= b1.id and b1.parent_id = b2.id and b.boundary_type_id = 2  and s.id = sg.institution_id and sg.id = stusg.student_group_id and sg.group_type='Class' and stusg.student_id = stu.id and stu.active = 2 and stusg.academic_id = 102 and stu.child_id = c.id and s.active=2
    and b2.id =9041
});
    $children->execute();
        ...
        foreach my $b1 (keys %{ $data->{hierarchy}{$b2} }) {
            foreach my $b (keys %{ $data->{hierarchy}{$b2}{$b1} }) {
                foreach my $sc (keys %{ $data->{hierarchy}{$b2}{$b1}{$b} }) {
                    ...
                    $tt->process('ang-koppalchildren.tt2', $tdata, "${b2}/${b1}/${b}/" . $filename) || warn $tt->error();
                }
            }
        }
    }
